...
export async function PATCH(
  ...
  if (target.role !== UserRole.LECTURER)
    return NextResponse.json(
      { error: "Only lecturer accounts can be managed via this route" },
      { status: 403 },
    );
  ...
  await prisma.auditLog.create({
    data: {
      adminId: admin.id,
      targetUserId: id,
      action: isSuspending ? "SUSPEND_LECTURER" : "REINSTATE_LECTURER",
      reason: data.reason,
    },
  });
  ...
  return NextResponse.json({
    message: `Lecturer account ${isSuspending ? "suspended" : "reinstated"}`,
  });
}
