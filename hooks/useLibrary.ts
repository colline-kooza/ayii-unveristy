...
// ── Newspapers ────────────────────────────
export function useNewspapers(filters: { page?: number } = {}) {
  const params = new URLSearchParams();
  if (filters.page) params.set("page", String(filters.page));

  return useQuery({
    queryKey: queryKeys.library.newspapers(filters),
    queryFn: async () => {
      const { data } = await apiClient.get(
        `/library/newspapers?${params.toString()}`,
      );
      return data;
    },
    placeholderData: keepPreviousData,
...
export function useUploadNewspaper() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (payload: any) => {
      const { data } = await apiClient.post("/library/newspapers", payload);
      return data;
    },
    onSuccess: () => {
...
export function useUpdateNewspaper() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...payload }: any) => {
      const { data } = await apiClient.patch(`/library/newspapers/${id}`, payload);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["library", "newspapers"] });
...
export function useDeleteNewspaper() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/library/newspapers/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["library", "newspapers"] });
      toast.success("Newspaper removed");
    },
    onError: (error) => toast.error(getErrorMessage(error)),
  });
}
