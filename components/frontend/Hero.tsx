...
export default function Hero() {
  return (
    <div className="relative min-h-screen flex items-center pt-24 md:pt-10 lg:pt-2 overflow-hidden bg-[#283593]">
      {/* Background with Gradient and Image */}
      <div className="absolute inset-0 z-0 overflow-hidden">
        <div className="relative w-full h-full opacity-40">
          <img
            src="https://img.freepik.com/free-photo/graduate-students-wearing-cap-gown-medium-shot_23-2148950540.jpg?ga=GA1.1.2016539446.1772535374&semt=ais_hybrid&w=740&q=80"
            alt="University Background"
            className="w-full h-full object-cover scale-110"
          />
        </div>
        {/* Gradient Overlay */}
        <div className="absolute inset-0 pointer-events-none bg-gradient-to-r from-[#283593] via-transparent to-transparent"></div>
        {/* Animated Glows */}
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 md:px-12 lg:px-24 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center py-12 lg:py-20">
        ...
        <div className="max-w-2xl transform animate-in fade-in slide-in-from-left duration-700">
          <div className="inline-flex items-center gap-3 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-white text-[11px] font-bold leading-none mb-6 hover:bg-white/10 transition-all cursor-default w-fit">
            <span className="bg-[#F4A800] text-[#283593] px-2 py-0.5 rounded-full uppercase font-extrabold text-[9px]">
              NEW
            </span>
            <span className="font-semibold tracking-wide">
              🎓 AYii University: Academic Hub
            </span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <Link href="/courses" className="w-full sm:w-auto">
              <Button
                size="lg"
                className="w-full sm:w-auto bg-[#0ee0f8] hover:bg-white text-black px-10 py-7 rounded-2xl font-black text-[12px] uppercase tracking-[0.2em] shadow-2xl shadow-[#0ee0f8]/20 transition-all hover:scale-[1.05] active:scale-95 gap-3"
              >
            ...
            <Link href="/dashboard" className="w-full sm:w-auto">
              <Button
                variant="outline"
                size="lg"
                className="w-full sm:w-auto border-2 border-white/20 bg-white/5 hover:bg-white/10 text-white font-black h-[64px] px-10 rounded-2xl transition-all text-[11px] uppercase tracking-[0.1em]"
              >
                Launch LMS <ArrowRight className="ml-2 w-4 h-4" />
              </Button>
            </Link>
          </div>
